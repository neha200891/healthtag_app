...
const AllTanentList = () => {
          ...
          if (response.status === true) {
            if (response.data.length > 0) {
              ...
              while (i < response.data.length) {
                arr.push({
                  business_name: response.data[i].tenantprofile?.business_name,
                  email: response.data[i].email,
                  phone_no: response.data[i].phone_no,
                  address: response.data[i].address,
                  incorporation_year: response.data[i].tenantprofile?.incorporation_year,
                  certification_number: response.data[i].tenantprofile?.certification_number,
  ...
  const columns = [
    {
      name: "Business Name",
      selector: (row) => row.tenantprofile.business_name,
    },
    {
      name: "Email",
    ...
    },
    {
      ...
      selector: (row) => row.tenantprofile.incorporation_year,
    },
    {
      name: "Certification Number",
      selector: (row) => row.tenantprofile.certification_number,
    },
    {
      name: "Health Provider",
      selector: (row) => row?.tenantprofile?.health_providers,
    },
    {
      name: "Users",
      selector: (row) => row?.tenantprofile?.users,
    },
    {
      name: "Status",
      cell: (user) => (
        ...
        <div className="col-md-12">
          <div className="card">
            <div className="card-header row">
              <h4 className="card-title">All Tenants List</h4>
